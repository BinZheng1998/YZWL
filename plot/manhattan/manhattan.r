setwd('E:/02-群体进化/06-羊/covergent/10kbwindow_5kbstep/DCMS/')
library(miamiplot)
vignette("miamiplot")
ggmiami2 <- function (data, split_by, split_at, chr = "chr", pos = "pos", 
                      p = "p", diff_y_scales = FALSE, chr_colors = c("black", 
                                                                     "grey"), upper_chr_colors = NULL, lower_chr_colors = NULL, 
                      upper_ylab = "-log10(p)", lower_ylab = "-log10(p)", genome_line = 0.00000005, 
                      genome_line_color = "red", suggestive_line = 0.00001, suggestive_line_color = "blue", 
                      hits_label_col = NULL, hits_label = NULL, top_n_hits = 5, 
                      upper_labels_df = NULL, lower_labels_df = NULL, upper_highlight = NULL, 
                      upper_highlight_col = NULL, upper_highlight_color = "green", 
                      lower_highlight = NULL, lower_highlight_col = NULL, lower_highlight_color = "green") 
{
  plot_data <- prep_miami_data(data = data, split_by = split_by, 
                               split_at = split_at, chr = chr, pos = pos, p = p, diff_y_scales = diff_y_scales)
  ...
  upper_plot <- ggplot2::ggplot() + ggplot2::geom_point(data = plot_data$upper, 
                                                        ...
                                                        size = 1) + ggplot2::scale_x_continuous(labels = plot_data$axis$chr, 
                                                                                                breaks = plot_data$axis$chr_center, expand = ggplot2::expansion(mult = 0.01), 
    ...
    ggplot2::theme(legend.position = "none", axis.title.x = ggplot2::element_blank(), 
                   plot.margin = ggplot2::margin(t = 10, b = 0, l = 10, 
                                                 r = 10))
  lower_plot <- ggplot2::ggplot() + ggplot2::geom_point(data = plot_data$lower, 
                                                        aes(x = .data$rel_pos, y = .data$logged_p, color = as.factor(.data$chr)), 
                                                        size = 1) + ggplot2::scale_x_continuous(breaks = plot_data$axis$chr_center, 
                                                                                                position = "top", expand = ggplot2::expansion(mult = 0.01)) + 
    ggplot2::scale_y_reverse(limits = c(plot_data$maxp_lower, 
                                        0), expand = ggplot2::expansion(mult = c(0, 0.02))) + 
...
df1 <- read.table('sheep_selection_neg_DCMS.txt',sep = '\t',header = T)
df1$logP <- -1
df2 <- read.table('sheep_selection_pos_DCMS.txt',sep = '\t',header = T)
df2$logP <- 1
data <- rbind(df1,df2)
...
colnames(data)[2] <- "pos"
head(data)
data$logpvalue <- log10(data$p_values)
p<-ggmiami2(data = data, 
            split_by = "logP", split_at = 0, p = "p_values", chr_colors = c("#257CB2", "#A6CEE3"),
            upper_ylab = "Domestic", suggestive_line = NULL,genome_line = 1e-02,
            lower_ylab = "Wild")

p
ggsave(p,filename = 'sheep_DCMS.png',dpi = 500,height = 8,width = 16)
