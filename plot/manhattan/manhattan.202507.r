setwd('~/project/01_evolution/convergent_evolution/07_result/20250714_DCMS_res/BW/plot/')
library(readxl)
library(dplyr)
library(tidyverse)

pos_dcms <- read.table('../../../20250714_DCMS_res/BW/cattle/cattle_BW_pos_DCMS_regions.txt',sep = '\t',header = T)
pos_dcms <- pos_dcms[pos_dcms$dcms > 0,]
neg_dcms <- read.table('../../../20250714_DCMS_res/BW/cattle/cattle_BW_neg_DCMS_regions.txt',sep = '\t',header = T)
...
p<-ggplot(Fst_pos, aes(x = BPcum, y = DCMS)) +
  ...
  scale_y_continuous(limits = c(-12,12),breaks = c(-10,-7.5,-5,-2.5,0,2.5,5,7.5,10),
                     labels = c("10","7.5","5.0","2.5","0","2.5","5.0","7.5","10"))+ 
  ...
  xlab("Chromosome")+
  ylab("BW low DCMS                           BW high DCMS")
p
ggsave(p,filename = 'cattle_BW_DCMS.pdf',dpi = 500,height = 6,width = 16)
