ggmiami4 <- function(data, split_by, split_at, chr = "chr", pos = "pos", 
                     ...
                     upper_y_lim = NULL, lower_y_lim = NULL, 
                     ...
                     lower_highlight_col = NULL, lower_highlight_color = "green") 
{
  ...
  # 计算统一的 x 轴范围
  x_range <- range(c(plot_data$upper$rel_pos, plot_data$lower$rel_pos), na.rm = TRUE)
  
  # 动态设置上图的 y 轴范围
  upper_y_max <- if (is.null(upper_y_lim)) plot_data$maxp_upper else upper_y_lim
  ...
  upper_plot <- ggplot2::ggplot() + 
    ...
    ggplot2::scale_y_continuous(limits = c(0, upper_y_max), 
                                expand = ggplot2::expansion(mult = c(0.02, 0))) + 
    ggplot2::labs(x = "", y = upper_ylab) + 
    ggplot2::theme_classic() + 
    ggplot2::theme(legend.position = "none", 
                   #axis.title.x = ggplot2::element_blank(), 
                   #axis.line.x = ggplot2::element_blank(),  # 移除上图 x 轴线
                   #axis.ticks.x = ggplot2::element_blank(), # 移除上图 x 轴刻度
                   plot.margin = ggplot2::margin(t = 10, b = -5, l = 10, r = 10)) # 底边距设为负值
  
  # 动态设置下图的 y 轴范围
  lower_y_max <- if (is.null(lower_y_lim)) plot_data$maxp_lower else lower_y_lim
  ...
  lower_plot <- ggplot2::ggplot() + 
    ...
    ggplot2::scale_x_continuous(limits = x_range, 
                                ...
                                expand = ggplot2::expansion(mult = 0.01), 
                                labels = NULL, 
                                position = "top") +  # x 轴移到顶部
    ggplot2::scale_y_reverse(limits = c(lower_y_max, 0), 
                             expand = ggplot2::expansion(mult = c(0, 0.02))) + 
    ggplot2::labs(x = "", y = lower_ylab) + 
    ggplot2::theme_classic() + 
    ggplot2::theme(legend.position = "none", 
                   axis.title.x = ggplot2::element_blank(), 
                   plot.margin = ggplot2::margin(t = -5, b = 10, l = 10, r = 10)) # 顶边距设为负值
  ...
  # 使用 gridExtra::grid.arrange 排列上下图
