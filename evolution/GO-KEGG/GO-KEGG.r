setwd('E:/02-群体进化/06-羊/selection_BW/10kb_window_5kb_step/DCMS-FST-PI/GO-KEGG/')
library(dplyr)
...
for (file in gene_files) {
  ...
  # 进行 GO 富集分析
  ego <- enrichKEGG(gene = id,organism = 'oas',keyType = "kegg",
                    pAdjustMethod = "BH",
                    pvalueCutoff = 1, 
                    qvalueCutoff = 1)
  ...
  # 生成输出文件名
  output_file <- sub("genes.txt", "KEGG.txt", basename(file))
