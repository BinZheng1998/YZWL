...
chr <- chr[chr$chr < 16 | chr$chr %in% c(17,18,19,20), ]
...
snp <-snp[snp$chr < 16 | snp$chr %in% c(17,18,19,20),]
snp <- na.omit(snp)
#pos selection
...
pos$chr <- as.numeric(pos$chr)
pos <-pos[pos$chr < 16 | pos$chr %in% c(17,18,19,20),]
...
neg$chr <- as.numeric(neg$chr)
neg <-neg[neg$chr < 16 | neg$chr %in% c(17,18,19,20),]


brk <- seq(0,200,by=25)*10^6
brk.label<-c()
for (i in brk){
...
brk.label[1]<-"0M"
brk.label <- c("0M","25M","50M","75M","100M","125M","150M","175M")
#
gradient_colors <- colorRampPalette(c("#dc0000ff","grey90", "#3c5488ff"))(nrow(chr))

pdf('big_chr_circlize.pdf')
circos.clear()
...
colors <- c("#dc0000ff", "grey90", "#3c5488ff")
leg1 <- Legend(col_fun =colorRamp2(c(0, 0.5, 1), colors),title='Chromosome',at = c(0,0.5,1),labels = c('','',''),direction = "horizontal")
colors2 <- c("#e64b35ff", "#00a087ff", "#4dbbd5ff")
leg2 <- Legend(labels = c('SNP density','Wild Regions','Domestic Regions'), legend_gp = gpar(fill =colors2), title = "")
pd <- packLegend(list = list(leg1, leg2),column_gap = unit(5, "mm"), row_gap = unit(0.5, "mm"))
