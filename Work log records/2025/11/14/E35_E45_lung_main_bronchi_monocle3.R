setwd('~/project/03_3D_spatial/02_result/251113_E35E45_lung_bronchial_monocle3/')
library(Seurat)
library(monocle3)
library(Matrix)
library(ggplot2)
...
obj_sub <- subset(obj,subset = celltype_gut %in% c('Lung Main Bronchi'))
...
head(obj_sub)
DimPlot(obj_sub)
FeaturePlot(obj_sub,features = c('TNC','AGR2'))
...
p + geom_vline(xintercept = seq(-5,-2.5,0.5)) + geom_hline(yintercept = seq(-3.5,0,0.5))
embed <- data.frame(Embeddings(obj_sub, reduction = "umap"))
embed <- subset(embed, umap_1 > -5 & umap_1 < -2.5 & umap_2 > -3.5 & umap_2 < 0)
