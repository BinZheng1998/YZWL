setwd('~/project/01_evolution/convergent_evolution/07_result/20251029_BWhigh_vs_low_pairwise_species/01result/dog')
library(dplyr)
high <- read.table('high_sample.txt',sep = '\t',header = T)
high$Breed <- gsub(' ','_',high$Breed)
#highsample <- read.table('~/project/01_evolution/convergent_evolution/04_dog/metadata/high_BW_dog.txt',sep = '\t')
#high2 <- high[high$sampleName %in% highsample$V1 ,]
high2 <- unique(high)
high2$phenotype <- 'high'
high3 <- high2[,c(2,1,3)]
colnames(high3) <- c('sample','variety','phenotype')

low <- read.table('low_sample.txt',sep = '\t',header = T)
low$Breed <- gsub(' ','_',low$Breed)
#lowsample <- read.table('~/project/01_evolution/convergent_evolution/04_dog/metadata/low_BW_dog.txt',sep = '\t')
#low2 <- low[low$sampleName %in% lowsample$V1 ,]
low2 <- unique(low)
low2$phenotype <- 'low'
low3 <- low2[,c(2,1,3)]
colnames(low3) <- c('sample','variety','phenotype')

dog_sample <- rbind(low3,high3)
write.table(dog_sample,'dog_BW_metadata.txt',row.names = F,sep = '\t',quote = F)
...
print(head(pos, 10))
write_tsv(pos, "dog_pairwise_pos_summary.tsv", na = "")
...
print(head(neg, 10))
write_tsv(neg, "dog_pairwise_neg_summary.tsv", na = "")
...
#画图
library(readr)
pos <- read_tsv('dog_pairwise_pos_summary.tsv')
pos$total_files <- max(pos$total_files)
pos$prop <- pos$sig_files/pos$total_files
n_pos <- nrow(pos)  
...
write_tsv(top5pos, "dog_pairwise_pos_top5_regions.tsv", na = "")

neg <- read_tsv('dog_pairwise_neg_summary.tsv')
neg$total_files <- max(neg$total_files)
neg$prop <- neg$sig_files/neg$total_files
n_neg <- nrow(neg)
...
write_tsv(top5neg, "dog_pairwise_neg_top5_regions.tsv", na = "")
...
p
ggsave(p,filename = 'dog_BW_pairwise_DCMS.pdf',dpi = 500,height = 6,width = 16)
