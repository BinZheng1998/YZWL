setwd('~/project/01_evolution/convergent_evolution/07_result/20250730_DCMS_GO/BW/cattle/')
library(clusterProfiler)
library(GO.db)
library(org.Bt.eg.db)
pos_gene <- read.table('../../../20250714_DCMS_res/BW/cattle//cattle_BW_pos_DCMS_top_001_merged_regions_50kb_genes.txt',sep = '\t',header = T)
...
genes <- unique(symbol)
genes
eg <- bitr(genes, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = "org.Bt.eg.db")
id <- as.character(eg[, 2])

ego <- enrichGO(gene = id, OrgDb = "org.Bt.eg.db", ont = "all",
                pAdjustMethod = "BH", pvalueCutoff = 1, qvalueCutoff = 1,
                readable = TRUE)

ego <- as.data.frame(ego)
ego <- ego[ego$pvalue < 0.05,]
write.table(ego,'cattle_DCMS_pos_top1_50kb_GO.txt',sep = '\t',row.names = F)
library(gground)
library(ggprism)
library(dplyr)
...
p
ggsave('cattle_DCMS_pos_top1_50kb_GO.pdf',p,width =10, height =8,dpi=300)
...
p1<-enrichmentNetwork(ego,colorBy = 'pvalue',
                      ...
                      verbose = F)
p1
ggsave('cattle_DCMS_pos_top1_50kb_GO_cluster.pdf',p1,width = 16,height = 12,dpi=300)
