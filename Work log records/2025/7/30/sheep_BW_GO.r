setwd('~/project/01_evolution/convergent_evolution/07_result/20250730_DCMS_GO/BW/sheep')
...
pos_gene <- read.table('../../../20250714_DCMS_res/BW/sheep/sheep_BW_pos_DCMS_top_001_merged_regions_50kb_genes.txt',sep = '\t',header = T)
...
genes <- unique(symbol)
genes
eg <- bitr(genes, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = sheep)
id <- as.character(eg[, 2])

ego <- enrichGO(gene = id, OrgDb = sheep, ont = "all",
                pAdjustMethod = "BH", pvalueCutoff = 1, qvalueCutoff = 1,
                readable = TRUE)

ego <- as.data.frame(ego)
ego <- ego[ego$pvalue < 0.05,]
write.table(ego,'sheep_DCMS_pos_top1_50kb_GO.txt',sep = '\t',row.names = F)
library(gground)
library(ggprism)
library(dplyr)
...
p
ggsave('sheep_DCMS_pos_top1_50kb_GO.pdf',p,width =14, height =8,dpi=300)
...
p1<-enrichmentNetwork(ego,colorBy = 'pvalue',
                      ...
                      verbose = F)
p1
ggsave('sheep_DCMS_pos_top1_50kb_GO_cluster.pdf',p1,width = 16,height = 12,dpi=300)
