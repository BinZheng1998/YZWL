...
# ==============================================================================
setwd('~/project/04_rnaseq/chicken/05_result/260120_rmbatcheffect_PCA/')
library(DESeq2)
library(sva)
library(limma)
library(ggplot2)
...
# --- 1.1 读取和清洗原始数据 ---
cat("正在读取大矩阵...\n")
df <- read.csv('../../03_stringtie_2.2.1/re_quant/gene_count_matrix.csv')
...
# --- 1.2 整理 Metadata ---
metadata <- read.table("../../metadata/chicken_rnaseq_metadata.txt",sep = '\t',header = T,fill=T)
metadata <- metadata[metadata$Filter == "PASS",]
metadata1 <- metadata[, c(2, 3, 11, 14)]
