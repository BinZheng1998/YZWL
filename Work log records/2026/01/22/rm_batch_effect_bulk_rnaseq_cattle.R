...
# ==============================================================================
setwd('/home/bzheng/project/04_rnaseq/cattle/2.result/260121_rm_batch_effect_PCA/')
library(DESeq2)
library(sva)
library(limma)
library(ggplot2)
...
# --- 1.1 读取和清洗原始数据 ---
cat("正在读取大矩阵...\n")
df <- read.table('../../1.data/cattle_total_merged_counts_matrix.txt', sep = '\t', header = T)
...
# --- 1.2 整理 Metadata ---
metadata <- read.table("../../metadata/cattle_metadata.txt",sep = '\t',header = T,fill=T)
metadata <- metadata[metadata$Filter == "PASS",]
metadata1 <- metadata[, c(1, 3, 7, 8)]
