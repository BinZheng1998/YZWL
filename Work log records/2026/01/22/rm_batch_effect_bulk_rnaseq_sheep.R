...
# ==============================================================================
setwd('/home/bzheng/project/04_rnaseq/sheep/2.result/260119_rm_batch_DESeq2_PCA')
library(DESeq2)
library(sva)
library(limma)
library(ggplot2)
...
# --- 1.1 读取和清洗原始数据 ---
cat("正在读取大矩阵...\n")
df <- read.table('../../1.data/sheep_total_merged_counts_matrix.txt', sep = '\t', header = T)
...
# --- 1.2 整理 Metadata ---
metadata <- read.table("../../1.data/metadata/sheepGTEx_metadata.txt",sep = '\t',header = T,fill=T)
#metadata <- metadata[metadata$Filter == "PASS",]
metadata <- metadata %>%
  filter(!str_detect(Filter, "PCA outlier"))
metadata1 <- metadata[, c(1, 2, 14, 20)]
...
for (tissue in target_tissues) {
  ...
  # --- 2.5 保存图片 ---
  pdf(paste0('20260120_SVA_', tissue, '.pdf'), width = 12, height = 5)
  print(p1 + p2)
  dev.off()
